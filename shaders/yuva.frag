...
layout(set = 2, binding = 3) uniform sampler2D u_tex_a;
    ...
    if (texelsPerPixel.x > 1.2 || texelsPerPixel.y > 1.2) {
        ...
        for (float y = -radius.y; y <= radius.y; y += 1.0) {
            for (float x = -radius.x; x <= radius.x; x += 1.0) {
                ...
                yuva += vec4(texture(u_tex_y, sampleUV).r, texture(u_tex_u, sampleUV).r, texture(u_tex_v, sampleUV).r, texture(u_tex_a, sampleUV).r) * w;
                totalWeight += w;
            }
        }
        yuva /= max(totalWeight, 1e-5);
    } else {
